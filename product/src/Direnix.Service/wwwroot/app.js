...
const I18N = {
  pt: {
    ...
    "metric.maxExposure": "Exposição máxima", "metric.activeRisks": "Riscos ativos",
  ...
  en: {
    ...
    "metric.maxExposure": "Max exposure", "metric.activeRisks": "Active risks",
...
function renderUsers(items) {
  ...
  body.innerHTML = items.map((u) => {
    ...
    // A própria conta troca a senha em "Minha conta" (o reset aqui encerraria a sessão).
    const resetBtn = u.isSelf ? "" : `<button class="secondary-button" type="button" data-reset="${escapeHtml(u.userId)}" data-name="${escapeHtml(u.username)}">${t("users.reset")}</button>`;
